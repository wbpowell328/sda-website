...
However, these projects demonstrated that it was not enough to plan the flows of generic trucks, but rather we had to model each individual driver. LOADMAP was extended to handle individual drivers for their first assignment, after which they were aggregated into individual trucks. An important property of the resulting model was that it could be solved as a "pure network" for which there were very fast algorithms (today, general purpose solvers such as Cplex handle these problems). The new model was programmed by David Cape for his undergraduate senior thesis, and was called "MicroMAP" reflecting its much higher level of detail. The resulting network is depicted to the right. A full research article describing MicroMAP was eventually published in the paper:
...
A separate paper provides additional background on this project, and the general problem of implementing real-time dispatch systems:
...
Schneider pioneered load matching in the late 1970's with a myopic model that ran in a batch model. However, Schneider uses a model based on approximate dynamic programming for strategic fleet planning. See [the Wagner page](/sda-website/wagner/).
{% endraw %}
