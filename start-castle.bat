@echo off
REM ============================================================
REM  CASTLE Lab — one-click local start
REM
REM  Double-click this file to start:
REM    - the website (http://localhost:4000/castle-website/)
...
echo.
echo ============================================
echo  Starting CASTLE Lab local site + chatbot
echo ============================================
echo.

REM --- Jekyll site (port 4000) ---------------------------------
echo Starting Jekyll site...
start "CASTLE Jekyll site" powershell -NoExit -Command ^
  "$env:PATH = 'C:\Ruby40-x64\bin;' + $env:PATH; ^
   cd '%ROOT%'; ^
   $env:RUBYOPT = '-r./ruby4_patch.rb'; ^
   Write-Host 'Starting Jekyll on http://localhost:4000/castle-website/ ...' -ForegroundColor Cyan; ^
   bundle exec jekyll serve --livereload --force_polling"

REM --- Chatbot server (port 3000) ------------------------------
echo Starting chatbot server...
start "CASTLE chatbot" powershell -NoExit -Command ^
...
start http://localhost:4000/castle-website/

echo.
echo ============================================
echo  Both servers are now running.
echo
echo   - Site:    http://localhost:4000/castle-website/
