...
// Dynamic decision-framing interview. The client sends the running
// conversation history (starting empty on turn 1) and receives either:
//   { action: 'ask',    question: '...' }              — the next follow-up
//   { action: 'finish', summary:  '...', reason: '...' } — done, here's context
// Stateless: server holds no per-user memory; the client persists the
// full transcript and re-sends it each turn. Enforced ~8-turn cap keeps
// runaway conversations from happening; the user can still tap "End
// interview & continue" at any point client-side.
  ...
  input_schema: {
    ...
    properties: {
      action: {
        type: 'string',
        enum: ['ask', 'finish'],
        description: '"ask" to ask another focused follow-up; "finish" to end the interview and hand off to the metrics/decisions/uncertainties steps.',
      },
      question: {
        type: 'string',
        description: 'ONE focused follow-up question (when action=ask). Must reference something the user just said — no scripted question lists. ≤ 40 words. Never ask two things in one turn.',
      },
      summary: {
        type: 'string',
        description: 'A compact summary of everything gathered (when action=finish), organized under these labelled sections (skip any that have nothing): DECISION MAKER, PROBLEM SETTING, RELEVANT HISTORY, GOALS, OTHER. 100–300 words total. Do NOT propose metrics, decisions, or uncertainties — those are the user\'s next steps.',
      },
      reason: {
        type: 'string',
        description: 'Very brief note (≤ 20 words) explaining why you\'re either asking this specific follow-up or ending the interview here. Used to help the user understand the flow.',
      },
    },
    required: ['action'],
  },
...
app.post('/framing/interview', framingLimiter, async (req, res) => {
  try {
    ...
    // Convert client shape ({role, text}) to Anthropic shape ({role, content}).
    // Drop malformed entries; clip individual messages so an accidental
    // giant paste doesn't blow the token budget.
    const msgs = [];
    ...
    // Anthropic requires messages start with 'user'. On turn 1 (empty
    // history) seed with a neutral opener so the assistant returns its
    // first question via the tool.
    const anthMessages = msgs.length === 0
      ? [{ role: 'user', content: 'Please begin the framing interview with your first question.' }]
      : msgs;
    // If the last turn is 'assistant', append a neutral user nudge — this
    // shouldn't happen in normal flow (client sends the user's answer,
    // then this endpoint replies with an assistant question) but the
    // Anthropic API rejects trailing-assistant messages.
    if (anthMessages[anthMessages.length - 1].role === 'assistant') {
      anthMessages.push({ role: 'user', content: '(no answer — proceed)' });
    }
    // Hard cap: after ~8 user turns, tell the model to finish. Guards
    // against a runaway interview even if the user keeps asking to
    // continue.
    ...
    if (!toolBlock) {
      return res.status(502).json({ error: 'Model did not respond. Try again.' });
    }
    ...
    // If the model said "ask" but produced no question (rare), fall back to
    // finish so the UI has something to render.
