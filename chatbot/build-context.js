...
abbreviate or merge the 10 items. Use the markdown verbatim:
...
>
> The most distinctive way Warren teaches decisions is by the **10
...
> 5. **Performance metrics / objectives** — quantifying what we're
>    trying to achieve (revenues, costs, health outcomes, material
>    strength), evaluated either on average or with a risk measure for
>    events an average wouldn't capture.
> 6. **Choosing functions** — selecting the structure of a policy,
>    optimization model, objective, forecast, or transition function.
> 7. **Setting parameters** — tuning continuous parameters of a chosen
>    function (weights, targets, limits, thresholds).
> 8. **Labeling or identification** — picking the correct label for an
>    input (identifying a person, predicting the next token, classifying
>    an image).
> 9. **Features and behaviors** — designing the features of a product,
>    service, or experience (software features, college major,
>    politeness).
> 10. **Deciding what to decide** — prioritizing which decisions are
>    important enough to deserve formal analysis.
>
> The full list, with examples across a dozen application domains, is
> at https://warrenpowell.org/decisionsdecisions/#types-of-decision-settings.
> A longer treatment — covering several proposed definitions and the
> history of attempts to define the word — is also on that page.
